...
import { Loader, LoaderTransparent } from "../../common";
...
export default function BoardForm({ projectId, isEdit, closeForm }) {
  ...
  const { working, board, loadingBoard } = useSelector(
  ...
  const handleSubmit = async (e) => {
    ...
    if (isEdit) {
      ...
      const result = await dispatch(createBoardAction(projectId, formData));
      if (result) closeForm();
    }
  };
